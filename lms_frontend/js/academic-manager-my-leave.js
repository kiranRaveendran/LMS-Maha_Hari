// ===============================================================
// Academic Manager — My Leave page. Submit leave requests to Admin
// and view their status/history. Same shape as js/leave-history.js
// (the Faculty equivalent), pointed at the Academic Manager's own
// endpoint and using the shared showToast()/extractApiError()
// helpers from academic-manager-common.js instead of a page-local
// successMessage box.
// ===============================================================
...
const AM_LEAVE_HISTORY_ENDPOINT = `${API.BASE_URL}/api/academic-manager/leave-history/`;
const PER_PAGE = 8;
...
async function submitLeaveRequest(e) {
    ...
    try {
        ...
        await api.post(AM_LEAVE_HISTORY_ENDPOINT, {
        ...
        showToast(extractApiError(error, "Failed to submit leave request."), true);
...
async function loadLeaveHistory() {
    ...
    try {
        ...
        const response = await api.get(AM_LEAVE_HISTORY_ENDPOINT);
...
// Inline confirm-in-row — same pattern used on the Faculty leave-history
// page, avoids stacking a second modal on top of anything.
...
async function deleteRequest(id) {
    ...
    try {
        ...
        await api.delete(`${AM_LEAVE_HISTORY_ENDPOINT}${id}/`);

        showToast("Leave request withdrawn.");
        loadLeaveHistory();

    } catch (error) {

        console.error(error);
        showToast(extractApiError(error, "Failed to withdraw request."), true);
